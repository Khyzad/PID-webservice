...
public class Minter {
    ...
    /**
     * Constructor for AutoMinters. In an AutoMinter scheme, the generation of
     * ids is dependent on the parameter RootLength and TokenType. The
     * AutoMinter generates a sequence of characters equal to the root length.
     * The type of characters used is based on TokenType.
     *
     ...
     * parameter is passed
     */
    public Minter(MinterServiceImpl DatabaseManager, String Prepend, TokenType TokenType,
    ...
    /**
     * Constructor for CustomMinters. In a CustomMinter scheme, the generation
     * of ids is dependent on the parameter CharMap. The CustomMinter will
     * generate as many characters as the length of CharMap. Each letter of the
     * CharMap dictates the range of characters in it's position.
     *
     * @param DatabaseManager Used to add and check ids against database.
     ...
     * parameter is passed
     */
    public Minter(MinterServiceImpl DatabaseManager, String Prepend, String CharMap, String Prefix,
     ...
     * database.
     */
    private Set<Pid> rollIds(Set<Pid> original, boolean order, long totalPermutations, long amount)
     ...
     */
    public Set<Pid> genIdAutoRandom(long amount) throws SQLException, IOException,
            NotEnoughPermutationsException, BadParameterException {

        String tokenMap = BaseMap.get(TokenType);

        Set<Pid> tempIdList = new LinkedHashSet((int) amount);
     ...
     * parameter is passed
     */
    public Set<Pid> genIdAutoSequential(long amount)
     ...
     * parameter is passed
     */
    public Set<Pid> genIdCustomRandom(long amount)
            throws SQLException, IOException, BadParameterException,
            NotEnoughPermutationsException {
     ...
     */
    public Set<Pid> genIdCustomSequential(long amount)
    ...
    public MinterServiceImpl getDatabaseManager() {
    ...
    private class AutoId extends Pid {
        ...
        public String getName() {
            String rootName = "";
            for (int i = 0; i < this.getBaseMap().length; i++) {
                rootName += TokenMap.charAt(this.getBaseMap()[i]);
            }
 ...
 MinterServiceImpl, to every possible name an Pid can have.
         *
         * @return - the name of an Pid.
         */
        @Override
        public String getRootName() {
            String charId = "";
        ...
        public String getName(){            
            String rootName = "";
            for (int i = 0; i < this.getBaseMap().length; i++) {
                rootName += TokenMapArray[i].charAt(this.getBaseMap()[i]);
            }
